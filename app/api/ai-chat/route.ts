...
async function sessionFor(request: Request) {
  const existing = request.headers.get("cookie")?.match(/(?:^|;\s*)supplierflow-chat=([^;]+)/)?.[1];
...
export async function POST(request: Request) {
  try {
    ...
    if (!process.env.DEEPSEEK_API_KEY) {
      ...
      return Response.json({ answer: cartAction ? `Added **${cartAction.quantity} × ${matches[0].name}** to your RFQ cart. Please complete your contact details on the right before sending.` : fallback, matches: publicMatches, action, cartAction, customer: inferredCustomer }, { headers: session.created ? { "Set-Cookie": `supplierflow-chat=${session.id}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000` } : {} });
    }
    ...
    let knowledge = "";
    try {
      const chunks = await (await supabase("knowledge_chunks?select=content,knowledge_documents!inner(title,source_type)&limit=200")).json() as KnowledgeChunk[];
      knowledge = fuseByKeywords(chunks, queries, (chunk: KnowledgeChunk) => chunk.content).slice(0, 3).map((chunk) => `${chunk.knowledge_documents.title}: ${chunk.content}`).join("\n");
    ...
    const answer = cartAction ? `Added **${cartAction.quantity} × ${top!.name}** to your RFQ cart. Complete your contact details on the right when you are ready to send it.` : wantsCart(question) ? fallback : ai?.choices?.[0]?.message?.content?.trim() || fallback;
    const completedCustomer = mergeCustomerDetails(inferredCustomer, answer);
    const cartActions = cartAction ? [cartAction] : cartActionsFromSummary(products, answer);
    await supabase("chat_messages", { method: "POST", body: JSON.stringify({ session_id: session.id, role: "assistant", content: answer }) });
    return Response.json({ answer, matches: publicMatches, action, cartAction: cartActions[0] || null, cartActions, customer: completedCustomer }, { headers: session.created ? { "Set-Cookie": `supplierflow-chat=${session.id}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000` } : {} });
