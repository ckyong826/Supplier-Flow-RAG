...
export async function POST(request: Request) {
  try {
    ...
    await supabase("products?is_active=eq.true", { method: "PATCH", body: JSON.stringify({ is_active: false, updated_at: new Date().toISOString() }) });
    ...
    return Response.json({ archived: existing.length, added: officialProducts.length });
